...
This script needs to be run after first-setup.sql as it presumes the existence of the IDI_Sample.data schema.
(Stored procedures need to be attached to a particular schema).
...
IF OBJECT_ID('IDI_Sample.data.copy_sample') IS NOT NULL
	DROP PROCEDURE data.copy_sample
GO



CREATE PROCEDURE data.copy_sample (@schema VARCHAR(200), @tab VARCHAR(200))
AS
BEGIN
