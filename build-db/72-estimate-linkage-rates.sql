...
and quality of data for each variable.  These "totals" are the cumulative products of the individual tables' linkage rates.  Why?  Because
...
only 0.5 * 1.0 = 0.5, not mean(0.5, 1.0) = 0.75. 
...
-- To estimate these total linkage rates we use the fact that the exponent of the sum of the logs
-- of a bunch of numbers is the same as multiplying them all together.  For example, this will turn
-- 0.5, 0.5, 0.5 into 0.125 (same as if we multiplied 0.5 x 0.5 x 0.5).  The more tables that are used
-- as data sources, the lower the data_linked_to_spine becomes.  Note this is a pretty crude process
-- and perhaps a little conservative (ie makes things look worse than they are):
SELECT
	variable_code,
	EXP(SUM(LOG(proportion_rows_on_spine))) AS data_linked_to_spine,
	EXP(SUM(LOG(proportion_snz_uid_on_spine))) AS snz_uid_linked_to_spine
INTO #tmp
