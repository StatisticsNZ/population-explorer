/*
Adds days spent in education per year to the fact table. 

Aggregates IDI_Sandpit.intermediate.days_in_education table (days in education per snz_uid per year-month) to days per year.
...
FROM
	(SELECT 
		...
		CASE 
			...
			WHEN education_days <= 90 AND education_days > 0	THEN '1 to 90 days'
