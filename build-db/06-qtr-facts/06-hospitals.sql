...
INSERT INTO IDI_Sandpit.pop_exp_dev.dim_explorer_value_qtr
		...
		VALUES
		 ...
		 ('two to three discharges', @var_code, 2),
		 ('four or more discharges', @var_code, 3);
...
FROM
	(SELECT
		...
		CASE WHEN value = 1 THEN 'one discharge'
			 WHEN value > 1 AND value < 4 THEN 'two to three discharges'
			 WHEN value >= 4 THEN 'four or more discharges'
		END AS value_char
	FROM 
		(SELECT  
			COUNT(*) as value,
