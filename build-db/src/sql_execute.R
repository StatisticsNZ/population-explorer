...
sql_execute <- function(channel, filename, sub_in = NULL, sub_out = NULL, fixed = TRUE, 
                        error_action = "stop", log_table = "IDI_Sandpit.dbo.pop_exp_build_log", 
                        verbose = TRUE, ...){
  ...
  # do the find and replace if needed
  if(!is.null(sub_in)){
  ...
  base_log_entry <- data.frame(
    ...
    target_schema = ifelse(grepl("^int", filename), "intermediate", sub_out),
  ...
  for(i in 1:length(sql_split)){
     ...
     res <- sqlQuery(channel, sql_split[[i]], ...)
     ...
     if(!is.null(log_table)){
       ...
       sql <- with(log_entry, paste0("INSERT INTO ", 
                                     log_table, 
                                     "(start_time, end_time, target_schema, script_name, batch_number, result, err_mess)",
